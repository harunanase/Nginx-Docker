...
RUN apk add --no-cache g++ pcre-dev zlib-dev linux-headers openssl-dev libmaxminddb-dev geoip-dev make
...
RUN sed -i "22s/NGINX_VER/\"${NGINX_NAME}\"/" ./src/http/ngx_http_special_response.c
RUN sed -i "29s/NGINX_VER_BUILD/\"${NGINX_NAME}\"/" ./src/http/ngx_http_special_response.c
RUN sed -i "36s/nginx/${NGINX_NAME}/" ./src/http/ngx_http_special_response.c
...
RUN cd /build/lua_resty_lrucache && make -j$(nproc) && make install PREFIX=${prefix}
